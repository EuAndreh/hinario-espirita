...
(defun %gen-boneco-tmps (fname n-pages l-center r-center pg-counter)
  (labels ((tmp-str (n)
           ...
           (r-pdf (n counter)
             ...
             (pdf n counter))
           (pdf (n counter)
             ´cd ?*path* && pdfseparate -f ?n -l ?n ?fname ?(tmp-str counter)´))
...
(defun hinarios (musicas tonalidades)
  (let* ((date-str (format-timestring nil (now) :format #l"data"))
    ...
    (mp [spit (tex _) (merge-pathnames (name _))]
        (ht-keys %scaffolds))
    ...
    (mp #'gen-hinarios '( ;; "Hinário" "Cifras" "Transparências" "Slides"
                         ;; "HinárioBoneco"
                         "CifrasBoneco"))
...
(defun hinarios (musicas tonalidades)
  (let* ((rel-path (fmt "../static/pdf/~a/~a/"
                        (format-timestring nil (now) :format #l"data")
                        (uuid:make-v4-uuid)))
         (cwd (uiop:getcwd))
         (*default-pathname-defaults* (car (directory (ensure-directories-exist rel-path))))
         (tex-dir (ensure-directories-exist (merge-pathnames "tex/"))))
    (uiop:chdir *default-pathname-defaults*)
    (copiar-e-transpor-musicas tonalidades tex-dir)
    ;; escrever no idioma cerot
    (mp [spit (tex _) (merge-pathnames (name _))]
        (ht-keys %scaffolds))
    (dolist (f (directory (merge-pathnames (fmt "static/pdf/i18n/~a/*.pdf" *i18n-route*)
                                           *application-root*)))
      (copy-file f (merge-pathnames (& (pathname-name f) ".pdf"))
                 :overwrite t))
    (gen-musicas-list musicas)
    (mp [gen-hinarios (tex _)]
        (ht-keys %scaffolds))
    (boneco :hinario-boneco)
    (boneco :cifras-boneco)
    (unless (eq *locale* :pt)
      (dolist (fname '("Hinário" "Cifras" "Transparências" "Slides"
                       "Hinário(boneco)" "Cifras(boneco)"))
        (copy-file (merge-pathnames (& fname ".pdf"))
